...
if __name__ == "__main__":
    ...
    for sub in ["sa_traj500", "loss_csv500", "loss_plot500", "temp_csv500", "heatmaps500"]:
    ...
    summary_csv = os.path.join(BASE, "htc_summary500_sa.csv")
    if os.path.exists(summary_csv):
        os.remove(summary_csv)

    for i in range(1, 31):
        for j in range(1, 21):
            folder = f"3/{i}-{j}-HTC500"
            ...
            df.to_csv(f"{BASE}/sa_traj500/sa_traj_{case}.csv", index=False)
            df.to_csv(f"{BASE}/loss_csv500/loss_log_{case}.csv", index=False)
            ...
            plt.legend(); plt.grid()
            plt.savefig(f"{BASE}/loss_plot500/loss_plot_{case}.png")
            plt.close()
            ...
            with open(summary_csv, "a", newline="") as f:
                ...
                w.writerow([case, htc_pred, 500, abs(htc_pred-500), t])

            # ---- temperature prediction for ALL time CSVs ----
            os.makedirs(f"{BASE}/temp_csv500/{case}", exist_ok=True)
            os.makedirs(f"{BASE}/heatmaps500/{case}", exist_ok=True)
            ...
            for csv_name in sorted(os.listdir(folder)):
                ...
                pd.DataFrame(rows, columns=["row","col","true_temp","pred_temp","abs_error"]) \
                    .to_csv(f"{BASE}/temp_csv500/{case}/{csv_name}", index=False)
                ...
                plt.tight_layout()
                plt.savefig(f"{BASE}/heatmaps500/{case}/{csv_name.replace('.csv','.png')}")
                plt.close()
