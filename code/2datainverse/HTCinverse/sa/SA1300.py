...
if __name__ == "__main__":
    ...
    for sub in ["sa_traj1300", "loss_csv1300", "loss_plot1300", "temp_csv1300", "heatmaps1300"]:
    ...
    summary_csv = os.path.join(BASE, "htc_summary1300_sa.csv")
    if os.path.exists(summary_csv):
        os.remove(summary_csv)

    for i in range(1, 31):
        for j in range(1, 21):
            folder = f"3/{i}-{j}-HTC1300"
            ...
            df.to_csv(f"{BASE}/sa_traj1300/sa_traj_{case}.csv", index=False)
            df.to_csv(f"{BASE}/loss_csv1300/loss_log_{case}.csv", index=False)
            ...
            plt.legend(); plt.grid()
            plt.savefig(f"{BASE}/loss_plot1300/loss_plot_{case}.png")
            plt.close()
            ...
            with open(summary_csv, "a", newline="") as f:
                ...
                w.writerow([case, htc_pred, 1300, abs(htc_pred-1300), t])

            # ---- temperature prediction for ALL time CSVs ----
            os.makedirs(f"{BASE}/temp_csv1300/{case}", exist_ok=True)
            os.makedirs(f"{BASE}/heatmaps1300/{case}", exist_ok=True)
            ...
            for csv_name in sorted(os.listdir(folder)):
                ...
                pd.DataFrame(rows, columns=["row","col","true_temp","pred_temp","abs_error"]) \
                    .to_csv(f"{BASE}/temp_csv1300/{case}/{csv_name}", index=False)
                ...
                plt.tight_layout()
                plt.savefig(f"{BASE}/heatmaps1300/{case}/{csv_name.replace('.csv','.png')}")
                plt.close()
