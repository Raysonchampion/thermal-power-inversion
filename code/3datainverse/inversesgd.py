...
OUT_ROOT = "/home/yslu/0127/inverse_sgdepoch/"
...
for wl in workloads:
    ...
    for f in range(FRAME_START, FRAME_END):
        ...
        for ep in range(EPOCHS):
            # adaptive lambda (every 10 epochs) - OUTSIDE backward graph
            if ep % 10 == 0:
                with torch.no_grad():
            ...
            # backward + step
            loss_total.backward()
            torch.nn.utils.clip_grad_norm_([S], CLIP_NORM)
            opt.step()
            ...
            # save EVERY epoch to inverse_curve.csv
            curve_rows.append([
                ep,
                float(loss_T.item()),
                float(mae_n.item()),
            ...
            if ep % 50 == 0 or ep == EPOCHS - 1:
                print(f"{wl} frame {f} [ep {ep:04d}] "
                      ...
                      f"lam={float(lambda_prior_eff.item()):.2e} sum={loss_sum.item():.3e} "
                      f"smooth={loss_smooth.item():.3e} total={loss_total.item():.3e} "
                      f"Psum={Psum.item():.4f}W")

        # save per-epoch curve CSV
        ...
        # workload summary metrics (physical domain)
