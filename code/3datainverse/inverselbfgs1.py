...
OUT_ROOT = "/home/yslu/0127/inverse_lbfgsepochampere/"
os.makedirs(OUT_ROOT, exist_ok=True)

FRAME_START, FRAME_END = 0, 99   # use f -> f+1
...
# L-BFGS settings
EPOCHS = 200                 # outer steps
LBFGS_MAX_ITER = 20          # inner iterations per step (line-search)
EPS = 1e-8

# loss weights
ALPHA_PRIOR = 0.3            # adaptive prior ratio (prior contribution vs loss_T)
LAMBDA_POWER_SUM = 1.0       # enforce integral(P)=TOTAL_POWER_W
LAMBDA_SMOOTH = 0.02         # smoothness weight

# score weights (as requested)
W_MSE = 0.7
W_MAE = 0.3
...
print("[3/4] Start inversion (LBFGS) ...")
...
for wl in workloads:
    ...
    for f in range(FRAME_START, FRAME_END):
        ...
        for ep in range(EPOCHS):
            ...
            # update adaptive lambda OUTSIDE closure
            if ep % 10 == 0:
                with torch.no_grad():
                    Pn_tmp = torch.clamp(P_prior_n * F.softplus(S), 0.0, 1.0).float()
                    x_tmp = torch.stack([Tn, Pn_tmp, htc_map]).unsqueeze(0).float()
                    Tpred_tmp = model(x_tmp).squeeze(0).squeeze(0).float()

                    loss_T_tmp = mse_fn(Tpred_tmp, Tgt).detach()
                    loss_prior_tmp = (S ** 2).mean().detach()
                    lambda_prior_eff = (ALPHA_PRIOR * loss_T_tmp / (loss_prior_tmp + EPS)).float()
            ...
            def closure():
                ...
                # write cache
                loss_cache["loss_T"] = float(loss_T.detach().item())
                loss_cache["loss_prior"] = float(loss_prior.detach().item())
                loss_cache["loss_sum"] = float(loss_sum.detach().item())
                loss_cache["loss_smooth"] = float(loss_smooth.detach().item())
                loss_cache["loss_total"] = float(loss_total.detach().item())
                loss_cache["Psum"] = float(Psum.detach().item())

                loss_cache["mae_n"] = float(mae_n.detach().item())
                loss_cache["mse_n"] = float(mse_n.detach().item())
                loss_cache["score_n"] = float(score_n.detach().item())
                loss_cache["mae_r"] = float(mae_r.detach().item())
                loss_cache["mse_r"] = float(mse_r.detach().item())
                loss_cache["score_r"] = float(score_r.detach().item())
            ...
            curve_rows.append([
                ...
                loss_cache["mse_r"],
                loss_cache["score_r"],
                loss_cache["loss_prior"],
                float(lambda_prior_eff.item()),
                loss_cache["loss_sum"],
                loss_cache["loss_smooth"],
                loss_cache["loss_total"],
                loss_cache["Psum"]
            ])

            if ep % 10 == 0 or ep == EPOCHS - 1:
                print(f"{wl} frame {f} [ep {ep:04d}] "
                      f"T={loss_cache['loss_T']:.3e} "
                      f"norm(MAE={loss_cache['mae_n']:.3e}, MSE={loss_cache['mse_n']:.3e}, S={loss_cache['score_n']:.3e}) "
                      f"real(MAE={loss_cache['mae_r']:.3e}C, MSE={loss_cache['mse_r']:.3e}C2, S={loss_cache['score_r']:.3e}) "
                      f"lam={float(lambda_prior_eff.item()):.2e} sum={loss_cache['loss_sum']:.3e} "
                      f"smooth={loss_cache['loss_smooth']:.3e} total={loss_cache['loss_total']:.3e} "
                      f"Psum={loss_cache['Psum']:.4f}W")

        # save curve (FULL, every epoch)
